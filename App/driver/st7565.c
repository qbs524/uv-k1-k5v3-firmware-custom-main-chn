...
uint8_t cmds[] = {
    ...
    void ST7565_Gauge(uint8_t line, uint8_t min, uint8_t max, uint8_t value)
    {
        ...
        gFrameBuffer[line][54-adj] = 0x0c;
        gFrameBuffer[line][55-adj] = 0x12;

        gFrameBuffer[line][121-adj] = 0x12;
        gFrameBuffer[line][122-adj] = 0x0c;

        uint8_t filled = map(value, min, max, 56-adj, 120);

        for (uint8_t i = 56-adj; i <= 120-adj; i++) {
