...
void UI_PrintString(const char *pString, uint8_t Start, uint8_t End, uint8_t Line, uint8_t Width)
{
    ...
    for (i = 0; i < Length; i++)
    {
        ...
        else if(pString[i] >= 0xb0 && pString[i] > 0xa0 && pString[i] < 0xf8) //check chinese font?
        {
            ChnTableAddr = ((pString[i] - 0xb0) * 94 + pString[i+1]-0xa1)*32 + 0x111340;
            PY25Q16_ReadBuffer(ChnTableAddr, ChnFont, 32);
            memcpy(gFrameBuffer[Line + 0] + ofs, &ChnFont[0+1], 15);
            memcpy(gFrameBuffer[Line + 1] + ofs, &ChnFont[16+1], 15);
            i++;
            ofs   = ofs + 15;
        }
    }
