...
//   text,          menu ID
    {"步进频率",        MENU_STEP          },
    {"发射功率",       MENU_TXP           }, // was "TXP"
    {"接收数字亚音",       MENU_R_DCS         }, // was "R_DCS"
    {"接收模拟亚音",      MENU_R_CTCS        }, // was "R_CTCS"
    {"发送数字亚音",       MENU_T_DCS         }, // was "T_DCS"
    {"发送模拟亚音",      MENU_T_CTCS        }, // was "T_CTCS"
    {"频差方向",      MENU_SFT_D         }, // was "SFT_D"
    {"频差频率",      MENU_OFFSET        }, // was "OFFSET"
    {"宽窄带",         MENU_W_N           },
#ifndef ENABLE_FEAT_F4HWN
    {"Scramb",      MENU_SCR           }, // was "SCR"
#endif
    {"遇忙禁发",      MENU_BCL           }, // was "BCL"
    {"压扩",      MENU_COMPAND       },
    {"调制模式",        MENU_AM            }, // was "AM"
#ifdef ENABLE_FEAT_F4HWN
    {"发射锁",      MENU_TX_LOCK       }, 
    ...
    {"自动锁键",      MENU_AUTOLK        }, // was "AUTOLk"
    {"发送超时",      MENU_TOT           }, // was "TOT"
    {"省电模式",      MENU_SAVE          }, // was "SAVE"
    {"电量显示",      MENU_BAT_TXT       },
    {"麦克风增益",         MENU_MIC           },
    {"麦克风电平",      MENU_MIC_BAR       },
    {"信道显示模式",      MENU_MDF           }, // was "MDF"
    {"开机显示",      MENU_PONMSG        },
    {"自动背光",      MENU_ABR           }, // was "ABR"
    {"最小背光亮度",       MENU_ABR_MIN       },
    {"最大背光亮度",       MENU_ABR_MAX       },
    {"背光点亮方式",      MENU_ABR_ON_TX_RX  },
    {"键音",        MENU_BEEP          },
#ifdef ENABLE_VOICE
    {"Voice",       MENU_VOICE         },
#endif
    ...
    {"D List",      MENU_D_LIST        },
#endif
    {"DTMF显示",      MENU_D_LIVE_DEC    }, // live DTMF decoder
#ifndef ENABLE_FEAT_F4HWN
    #ifdef ENABLE_AM_FIX
        {"AM Fix",      MENU_AM_FIX        },
    #endif
...
#endif
    {"电池调压",      MENU_BATCAL        }, // battery voltage calibration
    {"电池大小",      MENU_BATTYP        }, // battery type 1600/2200mAh
    {"参数复位",       MENU_RESET         }, // might be better to move this to the hidden menu items ?
...
const char* const gSubMenu_RXMode[] =
{
    ...
    "双信道接收发射", // Watch both and respond
    "主信道发副信道收",      // TX on main, RX on secondary
    "主信道发双信道收"  // always TX on main, but RX on both
};
...
const char gSubMenu_SCRAMBLER[][7] =
{
    ...
    const char gSubMenu_SET_TOT[][9] =  // Use by SET_EOT too
    {
        "关闭",
        "音频提示",
        "闪灯提示",
...
#endif
#ifdef ENABLE_FEAT_F4HWN
    {"切换接收模式",         ACTION_OPT_RXMODE},
    {"独占主信道收发",       ACTION_OPT_MAINONLY},
    {"发射键模式",             ACTION_OPT_PTT},
    {"切换宽/窄带",    ACTION_OPT_WN},
    //#if !defined(ENABLE_SPECTRUM) || !defined(ENABLE_FMRADIO)
    {"静音",            ACTION_OPT_MUTE},
    //#endif
    #ifdef ENABLE_FEAT_F4HWN_RESCUE_OPS
        {"一键设置高功率",    ACTION_OPT_POWER_HIGH},
        {"一键直频",  ACTION_OPT_REMOVE_OFFSET},
    #endif
#endif
};
...
void UI_DisplayMenu(void)
{
    ...
    const unsigned int menu_item_x1    = 0 ;//(8 * menu_list_width) + 2;
...
#ifdef ENABLE_FEAT_F4HWN
    //UI_DrawLineBuffer(gFrameBuffer, 0, 16, 127, 16, 1); // Be ware, status zone = 8 lines, the rest = 56 ->total 64
...
//          UI_PrintStringSmallNormal(String, 0, 0, 0);
    ...
    switch (UI_MENU_GetCurrentMenuId())
    {
        ...
        case MENU_OFFSET:
            ...
            {
                sprintf(String, "%3d.%05uMHz", gSubMenuSelection / 100000, abs(gSubMenuSelection) % 100000);
                UI_PrintString(String, menu_item_x1+16, menu_item_x2, 2, 8);
            }
            else
            {
                const char * ascii = INPUTBOX_GetAscii();
                sprintf(String, "%.3s.%.3sMHz ",ascii, ascii + 3);
                UI_PrintString(String, menu_item_x1+16, menu_item_x2, 2, 8);
            }
        ...
        case MENU_MEM_NAME:
        {
            ...
            if (valid)
            {
                ...
                if (!gAskForConfirmation && edit_index < 0)
                {   // show the frequency so that the user knows the channels frequency
                    sprintf(String, "%u.%05u", frequency / 100000, frequency % 100000);
        ...
        case MENU_S_LIST:
            if (gSubMenuSelection == 0)
                ...
                sprintf(String, "列表 [%u]", gSubMenuSelection);
            else if (gSubMenuSelection == 4)
                strcpy(String, "列表\n[1, 2, 3]");
        ...
        if (len > 0)
        {
            ...
            */
            ...
            if(UI_MENU_GetCurrentMenuId() == MENU_VOL)
            {
                ...
                UI_PrintStringSmallNormal("BA7IQE-v1.0", menu_item_x1, menu_item_x2, 4);
                //UI_PrintString("汉化", 0, 0, 4, 8);
            }
            else{
            // draw the text lines
    ...
    {   // display confirmation
        char *pPrintStr = (gAskForConfirmation == 1) ? "确认?" : "请等待!";
